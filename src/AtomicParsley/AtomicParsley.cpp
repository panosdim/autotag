...
#endif
char *file_progress_buffer = (char *) calloc(1, sizeof(char) *
...
void APar_TrackInfo(uint8_t &total_tracks, uint8_t &track_num,
                    short &codec_atom) { //returns the codec used for each 'trak' atom; used under Mac OS X only
...
short APar_FindLastLikeNamedAtom(char *atom_name, short containing_hierarchy) {
    short last_identically_named_atom = APar_FindLastChild_of_ParentAtom(
            containing_hierarchy); //default returns the last atom in the parent, not the parent
...
----------------------*/
void APar_CreateSurrogateAtom(AtomicInfo *surrogate_atom, const char *atom_name, uint8_t atom_level, uint8_t atom_class,
                              uint16_t atom_lang,
...
----------------------*/
AtomicInfo *
APar_FindAtom(const char *atom_name, bool createMissing, uint8_t atom_type, uint16_t atom_lang, bool match_full_uuids) {
    ...
    while (search_atom_name != NULL) {
        ...
        if (strlen(search_atom_name) == 4) {
            if (atom_type == UNKNOWN_ATOM) {
                known_atom = APar_MatchToKnownAtom(search_atom_name, last_known_present_parent->AtomicName, false,
                                                   atom_name);
        ...
        APar_CreateSurrogateAtom(&atom_surrogate, search_atom_name, present_atomic_level, search_atom_type, atom_lang,
                                 reverse_dns_name, revdns_name_len);
        atom_index = 0;
            ...
            if (iter == 0 && createMissing) {
                ...
                if (last_known_present_parent != NULL) {
                    ...
                    if (search_atom_type == PACKED_LANG_ATOM) {
                        last_hierarchical_atom = APar_FindLastLikeNamedAtom(atom_surrogate.AtomicName,
                                                                            last_known_present_parent->AtomicNumber);
                    } else {
                        last_hierarchical_atom = APar_FindLastChild_of_ParentAtom(
                                last_known_present_parent->AtomicNumber);
                    }
                    thisAtom = APar_CreateSparseAtom(&atom_surrogate, last_known_present_parent,
...
void APar_AtomicRead(short this_atom_number) {
    ...
    parsedAtoms[this_atom_number].AtomicData = (char *) malloc(
            sizeof(char) * (size_t) (parsedAtoms[this_atom_number].AtomicLength));
    memset(parsedAtoms[this_atom_number].AtomicData, 0,
           sizeof(char) * (size_t) (parsedAtoms[this_atom_number].AtomicLength));
...
#else
        unsigned char *utf8_data = Convert_multibyteUTF16_to_UTF8(unicode_string, (asset_length - 13) * 6,
                                                                  asset_length - 14);
        fprintf(stdout, "%s", utf8_data);
...
void APar_print_ISO_UserData_per_track() {
    ...
    APar_TrackInfo(total_tracks, a_track,
                   an_atom); //With track_num set to 0, it will return the total trak atom into total_tracks here.
    ...
    for (uint8_t i = 1; i <= total_tracks; i++) {
        ...
        if (trak_udtaAtom != NULL &&
            ...
            while (parsedAtoms[a_trak_atom].AtomicLevel ==
                   trak_udtaAtom->AtomicLevel + 1) { //only work on moov.trak[i].udta's child atoms
                ...
                uint16_t packed_lang = APar_read16(bitpacked_lang, source_file,
                                                   parsedAtoms[a_trak_atom].AtomicStart + 12);
                APar_UnpackLanguage(unpacked_lang, packed_lang);

                APar_readX(box_data, source_file, parsedAtoms[a_trak_atom].AtomicStart + 14,
                           box_length - 14); //4bytes length, 4 bytes name, 4 bytes flags, 2 bytes lang
        ...
        if (parsedAtoms[i].AtomicLevel == udtaAtom->AtomicLevel + 1) {
            ...
            switch (box) {
                ...
                case 0x616C626D : //'albm'
                {
                    ...
                    APar_readX(box_data, source_file, parsedAtoms[i].AtomicStart + 14,
                               box_length - 14); //4bytes length, 4 bytes name, 4 bytes flags, 2 bytes lang
                    ...
                    if (box ==
                        0x616C626D) { //'albm' has an *optional* uint8_t at the end for tracknumber; if the last byte in the tag is not 0, then it must be the optional tracknum (or a non-compliant, non-NULL-terminated string). This byte is the length - (14 bytes +1tracknum) or -15
                        if (box_data[box_length - 15] != 0) {
                            track_num = (uint16_t) box_data[box_length - 15];
                            box_data[box_length -
                                     15] = 0; //NULL out the last byte if found to be not 0 - it will impact unicode conversion if it remains
                        }
                    }
                ...
                case 0x636C7366 : //'clsf'
                {
                    ...
                    APar_readX(box_data, source_file, parsedAtoms[i].AtomicStart + 12,
                               box_length - 12); //4bytes length, 4 bytes name, 4 bytes flags, 2 bytes lang
                ...
                case 0x6B797764 : //'kywd'
                {
                    ...
                    uint16_t packed_lang = APar_read16(bitpacked_lang, source_file,
                                                       parsedAtoms[i].AtomicStart + box_offset);
                    box_offset += 2;
                    ...
                    for (uint8_t x = 1; x <= keyword_count; x++) {
                        ...
                        APar_readX(keyword_data, source_file, parsedAtoms[i].AtomicStart + box_offset,
                ...
                case 0x6C6F6369 : //'loci' aka The Most Heinous Metadata Atom Every Invented - decimal meters? fictional location? Astromical Body? Say I shoot it on the International Space Station? That isn't a Astronimical Body. And 16.16 alt only goes up to 20.3 miles (because of negatives, its really 15.15) & the ISS is at 230 miles. Oh, pish.... what ever shall I do? I fear I am on the horns of a dilema.
                {
                    ...
                    fprintf(stdout, "[Long %lf", fixed_point_16x16bit_to_double(
                            APar_read32(float_buffer, source_file, parsedAtoms[i].AtomicStart + box_offset)));
                    box_offset += 4;
                    fprintf(stdout, " Lat %lf", fixed_point_16x16bit_to_double(
                            APar_read32(float_buffer, source_file, parsedAtoms[i].AtomicStart + box_offset)));
                    box_offset += 4;
                    fprintf(stdout, " Alt %lf ", fixed_point_16x16bit_to_double(
                            APar_read32(float_buffer, source_file, parsedAtoms[i].AtomicStart + box_offset)));
                    box_offset += 4;
                    free(float_buffer);
                    float_buffer = NULL;
                    ...
                    if (box_offset < box_length) {
                        ...
                        if (memcmp(box_data + box_offset - 14, "\xFE\xFF", 2) == 0) {
                            ...
                            box_offset += 2 * widechar_len(box_data + box_offset - 14, box_length - box_offset) +
                                          2; //*2 for utf16 (double-byte); +2 for the terminating NULL
                        } else {
...
void APar_Extract_uuid_binary_file(AtomicInfo *uuid_atom, const char *originating_file, char *output_path) {
    ...
    char *uuid_outfile = (char *) calloc(1, sizeof(char) * MAXPATHLEN +
                                            1); //malloc a new string because it may be a cli arg for a specific output path
    if (output_path == NULL) {
        char *orig_suffix = const_cast<char *>(strrchr(originating_file, '.'));
        if (orig_suffix == NULL) {
            fprintf(stdout,
                    "AP warning: a file extension for the input file was not found.\n\tGlobbing onto original filename...\n");
    ...
    if (outfile != NULL) {
        ...
        fclose(outfile);
        fprintf(stdout, "Extracted uuid=%s attachment (mime-type=%s) to file: ", uuid_atom->uuid_ap_atomname,
            ...
            if (thisAtom->AtomicVerFlags == (uint32_t) AtomFlags_Data_Text) {
                ...
                if ((memcmp(parent_atom_name, "trkn", 4) == 0) || (memcmp(parent_atom_name, "disk", 4) == 0)) {
                    ...
                    if (thisAtom->AtomicVerFlags == AtomFlags_Data_UInt &&
                        ...
                        char *uuid_description = (char *) calloc(1, sizeof(char) * descrip_len +
                        ...
                        char *file_suffix = (char *) calloc(1, sizeof(char) * suffix_len +
...
void APar_PrintDataAtoms(const char *path, char *output_path, uint8_t supplemental_info, uint8_t target_information) {
        ...
        if (strncmp(thisAtom->AtomicName, "data", 4) == 0) { //thisAtom->AtomicClassification == VERSIONED_ATOM) {
            ...
            if ((thisAtom->AtomicVerFlags == (uint32_t) AtomFlags_Data_Binary ||
                ...
                if (strncmp(parent->AtomicName, "----", 4) == 0) {
                    if (memcmp(parsedAtoms[i - 1].AtomicName, "name", 4) == 0) {
                        fprintf(stdout, "Atom \"%s\" [%s] contains: ", parent->AtomicName,
                                parsedAtoms[i - 1].ReverseDNSname);
                        APar_ExtractDataAtom(i);
                    }

                } else if (memcmp(parent->AtomicName, "covr", 4) ==
                           0) { //libmp4v2 doesn't properly set artwork with the right flags (its all 0x00)
                    artwork_count++;
            ...
            if (thisAtom->uuid_style == UUID_AP_SHA1_NAMESPACE) {
                ...
                if (target_information == PRINT_DATA) {
                    isolat1ToUTF8((unsigned char *) twenty_byte_buffer, 10,
                                  (unsigned char *) thisAtom->uuid_ap_atomname, 4);
    ...
    if (supplemental_info) {
        ...
        APar_Optimize(
                true); //just to know if 'free' atoms can be considered padding, or (in the case of say a faac file) it's *just* 'free'
...
void APar_PrintAtomicTree() {
    ...
    char *tree_padding = (char *) malloc(
            sizeof(char) * 126); //for a 25-deep atom tree (4 spaces per atom)+single space+term.
    ...
    while (true) {
        ...
        if (thisAtom->uuid_ap_atomname != NULL) {
            isolat1ToUTF8((unsigned char *) twenty_byte_buffer, 10, (unsigned char *) thisAtom->uuid_ap_atomname,
                          4); //converts iso8859 � in '�ART' to a 2byte utf8 � glyph
        } else {
            isolat1ToUTF8((unsigned char *) twenty_byte_buffer, 10, (unsigned char *) thisAtom->AtomicName,
                          4); //converts iso8859 � in '�ART' to a 2byte utf8 � glyph
        }
        ...
        if (thisAtom->AtomicLevel != 1) {
            for (uint8_t pad = 1; pad < thisAtom->AtomicLevel; pad++) {
                strcat(tree_padding,
                       "    "); // if the atom depth is over 1, then add spaces before text starts to form the tree
            }
            strcat(tree_padding, " "); // add a single space
        }

        if (thisAtom->AtomicLength == 0) {
            fprintf(stdout, "%sAtom %s @ %u of size: %u (%u*), ends @ %u\n", tree_padding, twenty_byte_buffer,
                    thisAtom->AtomicStart, ((uint32_t) file_size - thisAtom->AtomicStart), thisAtom->AtomicLength,
                    (uint32_t) file_size);
            ...
            if (thisAtom->uuid_style == UUID_AP_SHA1_NAMESPACE) {
                fprintf(stdout, "%sAtom uuid=", tree_padding);
                APar_print_uuid((ap_uuid_t *) thisAtom->AtomicName, false);
                fprintf(stdout, "(APuuid=%s) @ %u of size: %u, ends @ %u\n", twenty_byte_buffer, thisAtom->AtomicStart,
                        thisAtom->AtomicLength, (thisAtom->AtomicStart + thisAtom->AtomicLength));
            } else {
                fprintf(stdout, "%sAtom uuid=", tree_padding);
                APar_print_uuid((ap_uuid_t *) thisAtom->AtomicName, false);
                fprintf(stdout, " @ %u of size: %u, ends @ %u\n", thisAtom->AtomicStart, thisAtom->AtomicLength,
            ...
            if (UnicodeOutputStatus == WIN32_UTF16) {
                ...
                APar_fprintf_UTF8_data(twenty_byte_buffer);
                fprintf(stdout, " [%s] @ %u of size: %u, ends @ %u\n", unpacked_lang, thisAtom->AtomicStart,
                        thisAtom->AtomicLength, (thisAtom->AtomicStart + thisAtom->AtomicLength));
            } else {
                fprintf(stdout, "%sAtom %s [%s] @ %u of size: %u, ends @ %u\n", tree_padding, twenty_byte_buffer,
                        unpacked_lang, thisAtom->AtomicStart, thisAtom->AtomicLength,
                        (thisAtom->AtomicStart + thisAtom->AtomicLength));
            }
            ...
            if (UnicodeOutputStatus == WIN32_UTF16) {
                fprintf(stdout, "%sAtom ", tree_padding);
                APar_fprintf_UTF8_data(twenty_byte_buffer);
                fprintf(stdout, " @ %u of size: %u, ends @ %u", thisAtom->AtomicStart, thisAtom->AtomicLength,
                        (thisAtom->AtomicStart + thisAtom->AtomicLength));
            } else {
                fprintf(stdout, "%sAtom %s @ %u of size: %u, ends @ %u", tree_padding, twenty_byte_buffer,
                        thisAtom->AtomicStart, thisAtom->AtomicLength,
                        (thisAtom->AtomicStart + thisAtom->AtomicLength));
            }
    ...
    fprintf(stdout, "Total free atom space: %u bytes; %2.3lf%% waste.", freeSpace,
...
void APar_SimpleAtomPrintout() { //loop through each atom in the struct array (which holds the offset info/data)
    ...
    for (int i = 0; i < atom_number; i++) {
        ...
        fprintf(stdout, "%i  -  Atom \"%s\" (level %u) has next atom at #%i\n", i, thisAtom->AtomicName,
                thisAtom->AtomicLevel, thisAtom->NextAtomNumber);
    }
...
uint64_t APar_64bitAtomRead(FILE *file, uint32_t jump_point) {
    ...
    fread(sixtyfour_bit_data, 1, 8,
          file); //read in 16 bytes so we can do a valid extraction and do a nice atom data printout
    ...
    if (extended_dataSize > 4294967295UL - 30000000) {
        ...
        fprintf(stdout,
                "You must be off your block thinking I'm going to tag a file that is at LEAST %llu bytes long.\n",
                extended_dataSize);
...
----------------------*/
int
APar_MatchToKnownAtom(const char *atom_name, const char *atom_container, bool fromFile, const char *find_atom_path) {
    ...
    if (!parsedfile) {
        ...
        if (file != NULL) {
            ...
            uuid_info.uuid_AP_atom_name = (char *) malloc(
                    sizeof(char) * 5); //this will hold any atom name that is written after the uuid written by AP
            ...
            if (memcmp(atom, "ftyp", 4) ==
                0) { //jpeg2000 files will have a beginning 'jp  ' atom that won't get through here
                ...
                while (jump < (uint32_t) file_size) {
                    ...
                    if (dataSize == 0 && (atom[0] == 0 && atom[1] == 0 && atom[2] == 0 && atom[3] == 0)) {
                        gapless_void_padding = file_size -
                                               jump; //Apple has decided to add around 2k of NULL space outside of any atom structure starting with iTunes 7.0.0
                    ...
                    if (memcmp(atom, "data", 4) == 0 &&
                        ...
                        if (dataSize > parsedAtoms[atom_number - 1].AtomicLength) {
                            dataSize =
                                    parsedAtoms[atom_number - 1].AtomicLength - 8; //force its length to its true length
                            fprintf(stdout,
                                    "AtomicParsley warning: the 'data' child of the '%s' atom seems to be corrupted.\n",
                    ...
                    if (memcmp(atom, "uuid", 4) == 0) {
                        ...
                        if (UInt32FromBigEndian(uuid_info.binary_uuid + 8) == 0) { //the deperacted uuid form
                            ...
                            if (uuid_version == 5) {
                                ...
                                APar_generate_uuid_from_atomname(uuid_info.uuid_AP_atom_name,
                    ...
                    if (KnownAtoms[filtered_known_atom].box_type == VERSIONED_ATOM && !corrupted_data_atom) {
                        atom_verflags = UInt32FromBigEndian(
                                data + 8); //flags & versioning were already read in with the original 12 bytes
                    }
                    ...
                    if ((strncmp(atom, "mdat", 4) == 0) && (generalAtomicLevel == 1) && (dataSize == 1)) {
                        ...
                        APar_AtomizeFileInfo(jump, 1, extended_dataSize, atom, generalAtomicLevel,
                                             KnownAtoms[filtered_known_atom].container_state,
                                             KnownAtoms[filtered_known_atom].box_type, atom_verflags, atom_language,
                                             &uuid_info);

                    } else {
                        APar_AtomizeFileInfo(jump, dataSize, 0, atom, generalAtomicLevel,
                                             KnownAtoms[filtered_known_atom].container_state,
                                             corrupted_data_atom ? SIMPLE_ATOM
                                                                 : KnownAtoms[filtered_known_atom].box_type,
                                             atom_verflags, atom_language, &uuid_info);
                    }
                    ...
                    switch (KnownAtoms[filtered_known_atom].container_state) {
                        ...
                        case CHILD_ATOM : {
                            if ((generalAtomicLevel == 1) &&
                                (dataSize == 1)) { //mdat.length =1 64-bit length that is more of a cludge.
                        ...
                        case DUAL_STATE_ATOM : {
                            if (memcmp(atom, "meta", 4) == 0) {
                                ...
                                if (memcmp(container, "sinf", 4) ==
                                    0) { //seems for iTMS drm files, schi is a simple parent atom, and 'user' comes right after it
                                    jump += 8;
                                } else {
                    ...
                    if ((jump > 8 ? jump : 8) >=
                        (uint32_t) file_size) { //prevents jumping past EOF for the smallest of atoms
                        break;
                    }
...
short APar_InterjectNewAtom(char *atom_name, uint8_t cntr_state, uint8_t atom_class, uint32_t atom_length,
    ...
    new_atom->AtomicData = (char *) malloc(
            sizeof(char) * atom_length); //puts a hard limit on the length of strings (the spec doesn't)
...
----------------------*/
void APar_Unified_atom_Put(short atom_num, const char *unicode_data, uint8_t text_tag_style, uint32_t ancillary_data,
    ...
    if (unicode_data != NULL) {
        if (text_tag_style == UTF16_3GP_Style) {
            ...
            uint32_t glyphs_req_bytes = mbstowcs(NULL, unicode_data, string_length) *
            ...
            utf16_conversion = NULL;
                   ...
                   UTF8_iTunesStyle_Binary) { //because this will be 'binary' data (a misnomer for purl & egid), memcpy 4 bytes into AtomicData, not at the start of it
    ...
    if (metadata_style == ITUNES_STYLE) {
        ...
        if (strlen(atomPayload) == 0) {
            ...
            if (genre_number != 0) {
                ...
                APar_Unified_atom_Put(genreAtom->AtomicNumber, NULL, UTF8_iTunesStyle_256glyphLimited,
                                      (uint32_t) genre_number, 8);
...
void APar_MetaData_atomArtwork_Init(short atom_num, const char *artworkPath) {
    ...
    if (picture_size > 0) {
        ...
        uint32_t bytes_read = APar_ReadFile(parsedAtoms[atom_num].AtomicData + 4, artfile,
                                            (uint32_t) picture_size); //+4 for the 4 null bytes
...
void APar_MetaData_atomArtwork_Set(const char *artworkPath, char *env_PicOptions) {
    if (metadata_style == ITUNES_STYLE) {
        ...
        if (memcmp(artworkPath, "REMOVE_ALL", 10) == 0) {
            ...
            desiredAtom = APar_CreateSparseAtom(&sample_data_atom, desiredAtom,
...
----------------------*/
uint32_t APar_3GP_Keyword_atom_Format(char *keywords_globbed, uint8_t keyword_count, bool set_UTF16_text,
    ...
    for (uint8_t i = 1; i <= keyword_count; i++) {
        ...
        if (set_UTF16_text) {
            ...
            uint32_t glyphs_req_bytes = mbstowcs(NULL, a_keyword, string_len + 1) *
                                        2; //passing NULL pre-calculates the size of wchar_t needed;
            ...
            int bytes_converted = UTF8ToUTF16BE((unsigned char *) (formed_keyword_struct + formed_string_offset),
                                                glyphs_req_bytes, (unsigned char *) a_keyword, string_len);
...
APar_uuid_atom_Init
    ...
    uuid atoms are user-supplied/user-defined atoms that allow for extended tagging support. Because a uuid atom is malleable, and defined by the utility
		...
		//4bytes atom length, 4 bytes 'uuid', 16bytes uuidv5, 4bytes name of uuid in AP namespace, 4bytes versioning, 4bytes NULL, Xbytes data
----------------------*/
short APar_uuid_atom_Init(const char *atom_path, char *uuidName, const uint32_t dataType, const char *uuidValue,
                          bool shellAtom) {
    ...
    if (uuidValue == NULL || strlen(uuidValue) == 0) {
        ...
        desiredAtom->uuid_ap_atomname = (char *) calloc(1, sizeof(char) *
                                                           10);       //only useful to print out the atom tree midway through an operation
        memcpy(desiredAtom->uuid_ap_atomname, uuid_4char_name,
               4);               //only useful to print out the atom tree midway through an operation

        if (dataType == AtomFlags_Data_Text)
            APar_MetaData_atom_QuickInit(desiredAtom->AtomicNumber, dataType,
...
void APar_MetaData_atom_QuickInit(short atom_num, const uint32_t atomFlags, uint32_t supplemental_length,
    ...
    parsedAtoms[atom_num].AtomicLength =
            16 + supplemental_length; // 4bytes atom length, 4 bytes atom length, 4 bytes version/flags, 4 bytes NULL
...
short APar_MetaData_atom_Init(const char *atom_path, const char *MD_Payload, const uint32_t atomFlags) {
    ...
    AtomicInfo *desiredAtom = APar_FindAtom(atom_path, retain_atom, VERSIONED_ATOM,
                                            0); //finds the atom; if not present, creates the atom
    if (desiredAtom == NULL) return -1;
    modified_atoms = true;

    if (!retain_atom) {
        AtomicInfo *parent_atom = &parsedAtoms[APar_FindParentAtom(desiredAtom->AtomicNumber,
        ...
        parsedAtoms[desiredAtom->AtomicNumber].AtomicData = (char *) malloc(
                sizeof(char) * MAXDATA_PAYLOAD + 1); //puts a hard limit on the length of strings (the spec doesn't)
...
short APar_UserData_atom_Init(const char *atom_path, const char *UD_Payload, uint16_t UD_lang) {
    ...
    if (!retain_atom) {
        APar_RemoveAtom(atom_path, atom_type,
                        atom_type == VERSIONED_ATOM ? 1 : UD_lang); //find the atom; don't create if it's "" to remove
        return -1;
    } else {
        modified_atoms = true;
        ...
        parsedAtoms[desiredAtom->AtomicNumber].AtomicData = (char *) malloc(
                sizeof(char) * MAXDATA_PAYLOAD); //puts a hard limit on the length of strings (the spec doesn't)
...
----------------------*/
void APar_ISO_UserData_Set(char *iso_atom_name, const char *iso_payload, uint8_t iso_container, uint8_t iso_track,
                           uint16_t packed_lang, bool set_UTF16_text) {
    uint8_t total_tracks = 0;
    ...
    for (uint8_t i = 1; i <= total_settings; i++) {
        ...
        if (strlen(iso_payload) == 0) {
            APar_RemoveAtom(iso_atom_path, PACKED_LANG_ATOM,
                            packed_lang); //find the atom; don't create if it's "" to remove
        } else {
            modified_atoms = true;
            if (iso_container != MOVIE_LEVEL_ATOM)
            ...
            desiredAtom->AtomicData = (char *) malloc(
                    sizeof(char) * MAXDATA_PAYLOAD); //puts a hard limit on the length of strings (the spec doesn't)
    ...
    while (true) {
        if (parsedAtoms[stop_atom].AtomicLevel == 1) {
            byte_sum += (parsedAtoms[stop_atom].AtomicLength == 1
                         ? (uint32_t) parsedAtoms[stop_atom].AtomicLengthExtended
...
bool APar_Readjust_CO64_atom(uint32_t mdat_position, short co64_number) {
    ...
    for (uint32_t i = 1; i <= entries; i++) {
        ...
        if (i == 1 && mdat_supplemental_offset ==
                      0) { //for the first chunk, and only for the first *ever* entry, make the global mdat supplemental offset
            if (this_entry - removed_bytes_tally > mdat_position) {
                mdat_supplemental_offset =
                        (uint64_t) mdat_position - ((uint64_t) this_entry - (uint64_t) removed_bytes_tally);
        ...
        if (deduct) { //crap, uint32_t's were so nice to flip over by themselves to subtract nicely. going from 32-bit to 64-bit prevents that flipping
            ...
            this_entry +=
                    mdat_supplemental_offset + bytes_into_mdat; //this is where we add our new mdat offset difference
        }
...
bool APar_Readjust_TFHD_fragment_atom(uint32_t mdat_position, short tfhd_number) {
    ...
    if (parsedAtoms[tfhd_number].AtomicVerFlags &
        0x01) { //seems the atomflags suggest bitpacking, but the spec doesn't specify it; if the 1st bit is set...
        memset(tfhd_atomFlags_scrap, 0, 10);
...
void APar_ForcePadding(uint32_t padding_amount) {
    ...
    if (udta_dynamics.free_atom_repository) {
        ...
        parsedAtoms[udta_dynamics.free_atom_repository].AtomicData = (char *) malloc(
                sizeof(char) * padding_amount); //allocate memory to write the NULL space out)
        ...
        parsedAtoms[udta_dynamics.free_atom_secondary_repository].AtomicData = (char *) malloc(
                sizeof(char) * padding_amount);
        memset(parsedAtoms[udta_dynamics.free_atom_secondary_repository].AtomicData, 0, sizeof(char) * padding_amount);
    } else {
        APar_InterjectNewAtom("free", CHILD_ATOM, SIMPLE_ATOM, padding_amount, 0, 0, 1,
                              APar_FindLastChild_of_ParentAtom(udta_dynamics.moov_atom));
    }
...
void APar_ConsilidatePadding(uint32_t force_padding_amount) {
    if (force_padding_amount <= 8 ||
        tree_display_only) { //prevent having an atom of length 0 (which means it goes to EOF)
        return;
    }
    ...
    if (udta_dynamics.free_atom_repository) {
        ...
        APar_InterjectNewAtom("free", CHILD_ATOM, SIMPLE_ATOM, force_padding_amount, 0, 0, 1,
                              APar_FindLastChild_of_ParentAtom(udta_dynamics.moov_atom));
...
void APar_ForcePadding_sans_udta() {
    short total_file_level_atoms = APar_ReturnChildrenAtoms(0,
                                                            0); //'ftyp' (atom 0) is level 1, so this finds the count of level 1 atoms
    short eval_atom = 0;
    short moov_atom = 0;
    ...
    if (free_padding_space <= pad_prefs.minimum_required_padding_size && pad_prefs.default_padding_size >= 8) {
        APar_InterjectNewAtom("free", CHILD_ATOM, SIMPLE_ATOM, pad_prefs.default_padding_size, 0, 0, 1,
                              APar_FindLastChild_of_ParentAtom(moov_atom));
...
void APar_DetermineDynamicUpdate(bool initial_pass) {
    ...
    if (!initial_pass && !psp_brand) {
        if (udta_dynamics.free_atom_repository == 0) {
            ...
            udta_dynamics.free_atom_repository = APar_InterjectNewAtom("free", CHILD_ATOM, SIMPLE_ATOM,
                                                                       udta_dynamics.max_usable_free_space -
                                                                       (udtaAtom->AtomicLength -
                                                                        udta_dynamics.original_udta_size),
                                                                       0, 0, 1, udta_dynamics.last_udta_child_atom);

        } else {
            parsedAtoms[udta_dynamics.free_atom_repository].AtomicLength +=
                    udta_dynamics.contained_free_space - (udtaAtom->AtomicLength - udta_dynamics.original_udta_size);
        }
    }
    ...
    while (true) {
        ...
        if (iter == udta_dynamics.last_udta_child_atom ||
            iter == 0) { //this makes sure that anything in udta is processed...
    ...
    if (initial_pass) {
        int userdata_difference = udtaAtom->AtomicLength -
                                  udta_dynamics.original_udta_size; //if metadata became shorter or removed, free space would be created
    ...
    if (moov_atom > 0) {
        ...
        for (uint8_t moov_i = 1; moov_i <= total_moov_child_atoms; moov_i++) {
            ...
            if (memcmp(parsedAtoms[eval_atom].AtomicName, "udta", 4) == 0 && parsedAtoms[eval_atom].AtomicLevel == 2) {
                ...
                if (memcmp(parsedAtoms[eval_atom].AtomicName, "free", 4) !=
                    0) { //don't reorder just to move udta behind free
                    any_child_atom = eval_atom;
                }
            }
        }

        if (any_child_atom > udta_atom && !tree_display_only &&
            udta_atom > 0) { //memcmp(parsedAtoms[udta_atom].AtomicName, "udta", 4) == 0 works too
    ...
    while (true) {
        if (parsedAtoms[thisAtomNumber].AtomicLevel == 1) {
            ...
            if (parsedAtoms[thisAtomNumber].AtomicLength == 0) {
                new_file_size += (uint32_t) file_size -
                                 parsedAtoms[thisAtomNumber].AtomicStart; //used in progressbar; mdat.length = 1
            }
        }
        if (parsedAtoms[thisAtomNumber].NextAtomNumber == 0) {
            break;
    ...
    while (true) {
        ...
        while (parsedAtoms[next_atom].AtomicLevel >
            ...
            if (parsedAtoms[rev_atom_loop].AtomicLevel ==
                (parsedAtoms[next_atom].AtomicLevel - 1)) { // only child atoms 1 level down
        ...
        if (_atom_ == 0x75647461 && parsedAtoms[rev_atom_loop].AtomicLevel >
        ...
        if (_atom_ == 0x6D657461 && parsedAtoms[rev_atom_loop].AtomicLevel !=
                                    parsedAtoms[parsedAtoms[rev_atom_loop].NextAtomNumber].AtomicLevel -
                                    1) { //meta with no child atoms; get here by erasing the last existing uuid atom.
            parsedAtoms[rev_atom_loop].AtomicLength = 12;
        }
        if (_atom_ == 0x696C7374 && parsedAtoms[rev_atom_loop].AtomicLevel !=
                                    parsedAtoms[parsedAtoms[rev_atom_loop].NextAtomNumber].AtomicLevel -
                                    1) { //ilst with no child atoms; get here by erasing the last piece of iTunes style metadata
            parsedAtoms[rev_atom_loop].AtomicLength = 8;
        }
...
void APar_ValidateAtoms() {
    ...
    while (true) {
        ...
        if (parsedAtoms[iter].AtomicLength > (uint32_t) file_size && file_size > 300000) {
            if (parsedAtoms[iter].AtomicData == NULL) {
                fprintf(stderr,
                        "AtomicParsley error: an atom was detected that presents as larger than filesize. Aborting. %c\n",
                        '\a');
...
#else
                fprintf(stderr, "atom %s is %u bytes long which is greater than the filesize of %llu\n",
                        parsedAtoms[iter].AtomicName, parsedAtoms[iter].AtomicLength,
        ...
        if (parsedAtoms[iter].AtomicLevel == 1) {
            if (parsedAtoms[iter].AtomicLength == 0 && strncmp(parsedAtoms[iter].AtomicName, "mdat", 4) == 0) {
                ...
                simple_tally += parsedAtoms[iter].AtomicLength == 1 ? parsedAtoms[iter].AtomicLengthExtended
        ...
        if (memcmp(parsedAtoms[iter].AtomicName, "trak", 4) == 0 &&
            parsedAtoms[iter + 1].NextAtomNumber != 0) { //prevent writing any malformed tracks
...
void
APar_DeriveNewPath(const char *filePath, char *temp_path, int output_type, const char *file_kind, char *forced_suffix,
    ...
    if (output_type >= 0) {
		...
		memcpy(temp_path + base_len, file_kind, strlen(file_kind));
#else
        char *file_name = const_cast<char *>(strrchr(filePath, '/'));
...
void
APar_MergeTempFile(FILE *dest_file, FILE *src_file, uint32_t src_file_size, uint32_t dest_position, char *&buffer) {
    uint32_t file_pos = 0;
...
uint32_t APar_WriteAtomically(FILE *source_file, FILE *temp_file, bool from_file, char *&buffer, char *&conv_buffer,
                              uint32_t bytes_written_tally, short this_atom) {
    uint32_t bytes_written = 0;

    if (parsedAtoms[this_atom].AtomicLength > 1 &&
        parsedAtoms[this_atom].AtomicLength < 8) { //prevents any spurious atoms from appearing
            ...
            case CHILD_ATOM : {
                if (parsedAtoms[this_atom].AtomicClassification == EXTENDED_ATOM) {
                    ...
                    atom_data_size = parsedAtoms[this_atom].AtomicLength -
                                     (16 + 12); //16 uuid; 16 = 4bytes * ('uuid', ap_uuid_name, verflag, 4 NULL bytes)
...
void APar_WriteFile(const char *m4aFile, const char *outfile, bool rewrite_original) {
    ...
    if (temp_file != NULL) {
        ...
        while (true) {
            ...
            if (memcmp(thisAtom->AtomicName, "stco", 4) == 0) {
                ...
                temp_file_bytes_written += APar_WriteAtomically(source_file, temp_file, !readjusted_stco, file_buffer,
                                                                data, temp_file_bytes_written, thisAtomNumber);

            } else if (memcmp(thisAtom->AtomicName, "co64", 4) == 0) {
                bool readjusted_co64 = APar_Readjust_CO64_atom(mdat_position, thisAtomNumber);

                temp_file_bytes_written += APar_WriteAtomically(source_file, temp_file, !readjusted_co64, file_buffer,
                                                                data, temp_file_bytes_written, thisAtomNumber);
                ...
                temp_file_bytes_written += APar_WriteAtomically(source_file, temp_file, !readjusted_tfhd, file_buffer,
                                                                data, temp_file_bytes_written, thisAtomNumber);

            } else if (thisAtom->AtomicData != NULL || memcmp(thisAtom->AtomicName, "meta", 4) == 0) {
                temp_file_bytes_written += APar_WriteAtomically(source_file, temp_file, false, file_buffer, data,
                                                                temp_file_bytes_written, thisAtomNumber);
                ...
                if (thisAtom->AtomicContainerState <= SIMPLE_PARENT_ATOM) {
                    ...
                    temp_file_bytes_written += APar_WriteAtomically(source_file, temp_file, true, file_buffer, data,
                                                                    temp_file_bytes_written, thisAtomNumber);
                }
            }
        ...
        if (!udta_dynamics.dynamic_updating) {
            if (gapless_void_padding > 0 && pad_prefs.default_padding_size >
                                            0) { //only when some sort of padding is wanted will the gapless null padding be copied
    ...
    if (udta_dynamics.dynamic_updating) {
        ...
        APar_MergeTempFile(source_file, temp_file, temp_file_bytes_written,
                           parsedAtoms[udta_dynamics.udta_atom].AtomicStart, file_buffer);
        ...
        if (forced_suffix_type != NO_TYPE_FORCING) {
            ...
            if (forced_suffix_type ==
                FORCE_M4B_TYPE) { //using --stik Audiobook with --overWrite will change the original file's extension
                uint16_t filename_len = strlen(m4aFile);
                char *suffix = const_cast<char *>(strrchr(m4aFile, '.'));
